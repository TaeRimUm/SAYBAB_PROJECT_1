...
class Skul2: #오른쪽
    ...
    def load_images(self):
        if Skul2.images == None:
            ...
            for name in animation_names:
                Skul2.images[name] = [load_image("./Skul_1/" + name + " (%d)" % i + ".png") for i in range(1, 11)]
    ...
    def __init__(self, name='NONAME', x=0, y=0, size=1):
        ...
        self.size = size
        if Skul2.font is None:
            Skul2.font = load_font('ENCR10B.TTF', 16)
        self.load_images()
        ...
        self.x, self.y, self.fall_speed = random.randint(1580, 1590), random.randint(1, 750), 0
    ...
    def calculate_current_position(self):
        ...
        self.y = clamp(0, self.y, get_canvas_height() - 100)
    ...
    def draw(self):
        ...
        if math.cos(self.dir) < 0:
            if self.speed == 0:
                Skul2.images['Idle'][int(self.frame)].composite_draw(0, 'h', sx, sy, 100, 100)
            else:
                Skul2.images['Walk'][int(self.frame)].composite_draw(0, 'h', sx, sy, 100, 100)
        else:
            if self.speed == 0:
                Skul2.images['Idle'][int(self.frame)].draw(sx, sy, 100, 100)
            else:
                Skul2.images['Walk'][int(self.frame)].draw(sx, sy, 100, 100)
    ...
    def handle_collision(self, other, group):
        if group == 'Saybar:Skul2': #해골 입장에서 세이버가 부딪히면
